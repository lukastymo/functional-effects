...
object ZIOModel {
  ...
  object ZIO {
    ...
    def environment[R]: ZIO[R, Nothing, ZEnvironment[R]] = ZIO(env => Right(env))
  }
  ...
  final case class ZIO[-R, +E, +A](run: ZEnvironment[R] => Either[E, A]) { self =>
    def map[B](f: A => B): ZIO[R, E, B] = ZIO(env => run(env).map(f))

    def flatMap[R1 <: R, E1 >: E, B](f: A => ZIO[R1, E1, B]): ZIO[R1, E1, B] = ZIO { env =>
      run(env).flatMap(a => f(a).run(env))
    }

    def zip[R1 <: R, E1 >: E, B](that: ZIO[R1, E1, B]): ZIO[R1, E1, (A, B)] = ZIO { env =>
      for {
        a <- run(env)
        b <- that.run(env)
      } yield (a, b)
    ...
    def either: ZIO[R, Nothing, Either[E, A]] = ZIO(env => Right(run(env)))
...
object SuccessEffect extends ZIOAppDefault {
   ...
   * "Hello World".
   */
  val run = ZIO.succeed("Hello World")
}
...
object PrintSequenceZip extends ZIOAppDefault {
   ...
   */
  val run =
    Console.printLine("aaa") <*> (Console.printLine("bbb"))
//    Console.printLine("aaa").zip(Console.printLine("bbb"))
}
...
object PrintReadSequence extends ZIOAppDefault {
   ...
   */
  val run =
    Console.printLine("aaaa") *> Console.readLine("enter")
}
...
object SimpleDuplication extends ZIOAppDefault {
   ...
   */
  val run = {
    val hello = Console.printLine("Hello")
...
object FlatMap extends ZIOAppDefault {
  ...
  val run =
    Console.printLine("What is your name?") *>
      Console.readLine.flatMap(line => Console.printLine(s"Your name is: $line"))
}
...
object ForComprehension extends ZIOAppDefault {
  ...
  val run =
    for {
      _    <- Console.printLine("aaa")
      name <- Console.readLine
      _    <- Console.print(s"Your name ${name}")
    } yield ()
...
//    } yield ()
    ...
    Console.printLine("aaa").flatMap { _ =>
      readInt
        .flatMap(age => if (age < 18) Console.printLine("bbb") else Console.printLine("ccc"))
        .map(_ => ())
    }
  }
}
...
object AsyncExample extends ZIOAppDefault {
  ...
  lazy val loadBodyAsyncZIO: ZIO[Any, Throwable, String] =
    ZIO.async[Any, Throwable, String](cb => loadBodyAsync(body => cb(ZIO.succeed(body)), err => cb(ZIO.fail(err))))
